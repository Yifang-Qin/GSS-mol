...
# _AVG_DEGREE     = 75  # IS2RE: 100k, max_radius = 10, max_neighbors = 100, theoreticall, 33.684
...
class EquiformerV2(nn.Module):
    ...
    def __init__(
        ...
        time_embed_dim = self.sphere_channels_all * 4
        self.time_mlp = TimestepEmbedding(
            in_channels=timestep_input_dim,
            time_embed_dim=time_embed_dim,
            act_fn="silu",
            out_dim=self.sphere_channels_all,
        )
        ...
        self.with_bond_info = with_bondinfo
        if self.with_bond_info:
            self.bond_embedding = nn.Linear(4, self.edge_channels_list[0], bias=False)
    ...
    def forward(
        ...
        if timesteps.size() == torch.Size():
            timesteps = torch.atleast_1d(timesteps).repeat(num_graphs)

        timesteps = torch.repeat_interleave(timesteps, repeat)  # (N, )
        ...
        if self.with_bond_info:
            edge_distance = edge_distance + self.bond_embedding(bond_info)

        # Edge-degree embedding
        edge_degree = self.edge_degree_embedding(atomic_numbers, edge_distance, edge_index)
        x.embedding = x.embedding + edge_degree.embedding
        # Refer to https://github.com/atomicarchitects/equiformer_v2/issues/10#issuecomment-2022511661
        x.embedding[:, :1] = x.embedding[:, :1] + timestep_embed[:, None]
