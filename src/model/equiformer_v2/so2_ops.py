...
class SO2_Convolution(torch.nn.Module):
    ...
    def forward(self, x, x_edge):
        ...
        # print("self.mappingReduced.m_size[0]", self.mappingReduced.m_size[0]) # 4, on cuda
        if isinstance(self.mappingReduced.m_size[0], torch.Tensor):
            length = self.mappingReduced.m_size[0].item()
        else:
            length = self.mappingReduced.m_size[0]
        x_0 = x.embedding.narrow(1, 0, length)
        ...
        if isinstance(offset, torch.Tensor):
            offset = offset.cpu().item()
        
        for m in range(1, max(self.mmax_list) + 1):
            len_ = self.mappingReduced.m_size[m]
            if isinstance(len_, torch.Tensor):
                len_ = len_.cpu().item()
            
            # Get the m order coefficients
            x_m = x.embedding.narrow(1, offset, 2 * len_)
