...
class EquivariantRMSNormArraySphericalHarmonicsV2(nn.Module):
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__}(lmax={self.lmax}, num_channels={self.num_channels}, "
            f"eps={self.eps}, centering={self.centering}, std_balance_degrees={self.std_balance_degrees})"
        )
...
class EquivariantDegreeLayerScale(nn.Module):
    """
    1. Similar to Layer Scale used in CaiT (Going Deeper With Image Transformers (ICCV'21)),
    we scale the output of both attention and FFN.
    2. For degree L > 0, we scale down the square root of 2 * L, which is to emulate halving the
    number of channels when using higher L.
    """
